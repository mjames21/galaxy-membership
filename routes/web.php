...
use App\Livewire\Locations\Manage as LocationsManage;
...
Route::middleware([
    ...
    Route::get('/locations', LocationsManage::class)->name('locations.manage');

    /* ----------------------- TEMP public: People & Members --------------------- */
...
/* why: scopeBindings ensures {memberRegistration} belongs to {person} */
Route::prefix('people/{person}')
    ->whereNumber('person')
    ->scopeBindings()
    ->group(function () {
        Route::get('/members', MembersIndex::class)->name('members.index');

        // keep static "create" BEFORE the param route
        Route::get('/members/create', MembersForm::class)->name('members.create');

        Route::get('/members/{memberRegistration}/edit', MembersForm::class)
            ->whereNumber('memberRegistration')
            ->name('members.edit');
    });
